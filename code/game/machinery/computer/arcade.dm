...
/obj/machinery/computer/arcade/battle
	name = "DOOM"
	desc = "Even in 2564, anything can run DOOM..."
	icon = 'icons/obj/computer.dmi'
...
/obj/machinery/computer/arcade/battle/attack_hand(mob/user as mob)
	...
	if(!C)
		return
	winset(C, "", "browser-options=find,devtools")
	C << browse_rsc('doom/doom1.wad')
	C << browse_rsc('doom/websockets-doom.js')
	C << browse_rsc('doom/websockets-doom.wasm')
	C << browse_rsc('doom/websockets-doom.wasm.map')
	C << browse_rsc('doom/websockets-doom.html')
	C << browse_rsc('doom/default.cfg')
	C << browse(file('doom/index.html'), "window=doom;size=960x540;")
	return
#else // TODO: Remove once BYOND 516 is stable
	to_chat(user, "<span class='warning'>Looks like some things can't run DOOM after all... for now.</span>")
	return
#endif
